...
try {
  ...
  console.log('\n--------------------------------------------------');
  console.log(' Running Test Suite: Tiers 1 - 4 + Capture/Learning Policies (74 Test Cases)');
  console.log('--------------------------------------------------\n');

  // 2. Run the test suite via Node.js native test runner
  execSync('node --test test/tier1.test.js test/tier2.test.js test/tier3.test.js test/tier4.test.js test/capture-policy.test.js test/learning-fallback.test.js', { stdio: 'inherit' });
